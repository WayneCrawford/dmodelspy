function [uc1 uc2 uc3 jc1 jc2 jc3 kc1 kc2 kc3] = ok92fc(fault,csi,W,c,mu,lambda,U,delta,yyp,z)
% function [uc1 uc2 uc3 jc1 jc2 jc3 kc1 kc2 kc3] = ok92fb(fault,csi,W,c,mu,lambda,U,delta,yyp,z)
% Forward model of fault slip (uc), X-derivative (jc) and Y-derivative (kc) 
...
%
% OUTPUT
% The variables uc1 ... kC3 corrrespond to the top, middle and bottom equations
% in the third column of Table 6, table 7 and Table 8.
% *************************************************************************
...
if strcmpi(fault,'strike')==1
    ...
    uc1 = (1-alpha)*csi.*Y11*cos(delta) -alpha*csi.*q.*Z32;                             % first row, third column
    uc2 = (1-alpha)*(cos(delta)./R + 2*q.*Y11*sin(delta)) -alpha*ctilde.*q./R.^3;       % second row, third column
    uc3 = (1-alpha)*q.*Y11*cos(delta) -alpha*(ctilde.*eta./R.^3 -z.*Y11 + csi.^2.*Z32); % third row, third column
% *************************************************************************
    % STRIKE SLIP X-derivative: Okada (1992), Table 7, Type Tensile
    jc1 =  (1-alpha)*Y0*cos(delta)                               -alpha*q.*Z0;                                              % first row, first column
    jc2 = -(1-alpha)*csi.*(cos(delta)./R.^3+2*q.*Y32*sin(delta)) +alpha*3*ctilde.*csi.*q./R.^5;                             % second row, first column
    jc3 = -(1-alpha)*csi.*q.*Y32*cos(delta)                       +alpha*csi.*(3*ctilde.*eta./R.^5 -z.*Y32 -Z32 -Z0);       % third row, first column
% *************************************************************************
    % STRIKE SLIP Y-Derivative: Okada (1992), Table 8, Type Tensile
    kc1 = -(1-alpha)*csi.*P*cos(delta)...                                                
                          -alpha*csi.*Q;                                                                                    % first row, first column
    kc2 = 2*(1-alpha)*(dtilde./R.^3-Y0*sin(delta))*sin(delta) -ytilde*cos(delta)./R.^3 ...                                  
                          -alpha*((ctilde+dtilde)*sin(delta)./R.^3 -eta./R.^3 -3*ctilde.*ytilde.*q./R.^5);                  
    kc3 = -(1-alpha)*q./R.^3 +(ytilde./R.^3-Y0*cos(delta))*sin(delta) ...
                          +alpha*((ctilde+dtilde)*cos(delta)./R.^3 +3*ctilde.*dtilde.*q./R.^5 -(Y0*cos(delta)+q.*Z0)*sin(delta));              % third row, first column
% *************************************************************************
elseif strcmpi(fault,'dip')==1
    % DIP SLIP displacement: Okada (1992), Table 6, Type Dip
    uc1 = (1-alpha)*cos(delta)./R -q.*Y11*sin(delta)-alpha*ctilde.*q./R.^3;     % first row, third column
    uc2 = (1-alpha)*ytilde.*X11 -alpha*ctilde.*eta.*q.*X32;                     % second row, third column
    uc3 = -dtilde.*X11-csi.*Y11*sin(delta)-alpha*ctilde.*(X11-q.^2.*X32);       % third row, third column
% *************************************************************************
    % DIP SLIP X-Derivative: Okada (1992), Table 7, Type Dip
    jc1 = -(1-alpha)*csi*cos(delta)./R.^3 +csi.*q.*Y32*sin(delta) +alpha*3*ctilde.*csi.*q./R.^5;                       % first row, first column
    jc2 = -(1-alpha)*ytilde./R.^3                                 +alpha*3*ctilde.*eta.*q./R.^5;              % second row, first column
    jc3 =                             dtilde./R.^3 -Y0*sin(delta) +alpha*ctilde.*(1-3*q.^2./R.^2)./R.^3;                        % third row, first column
% *************************************************************************
    % DIP SLIP Y-Derivative: Okada (1992), Table 8, Type Dip
    kc1 = -(1-alpha)*eta./R.^3 +Y0*sin(delta).^2 ...
                                                 -alpha*((ctilde+dtilde)*sin(delta)./R.^3 -3*ctilde.*ytilde.*q./R.^5);             % first row, first column
    kc2 =  (1-alpha)*(X11-ytilde.^2.*X32)        -alpha*ctilde.*((dtilde+2*q*cos(delta)).*X32 - ytilde.*eta.*q.*X53);             % second row, first column
    kc3 = csi.*P*sin(delta) +ytilde.*dtilde.*X32 +alpha*ctilde.*((ytilde+2*q*sin(delta)).*X32 - ytilde.*q.^2.*X53);              % third row, first column
% *************************************************************************
else
    % TENSILE displacement: Okada (1992), Table 6, Type Tensile
    uc1 = -(1-alpha)*(sin(delta)./R +q.*Y11*cos(delta)) -alpha*(z.*Y11-q.^2.*Z32);      % first row, third column
    uc2 = (1-alpha)*2*csi.*Y11*sin(delta) +dtilde.*X11 -alpha*ctilde.*(X11-q.^2.*X32);                     % second row, third column
    uc3 = (1-alpha)*(ytilde.*X11+csi.*Y11*cos(delta)) +alpha*q.*(ctilde.*eta.*X32+csi.*Z32);       % third row, third column
% *************************************************************************
    % TENSILE X-derivative: Okada (1992), Table 7, Type Tensile
    jc1 =  (1-alpha)*csi*sin(delta)./R.^3 +csi.*q.*Y32*cos(delta) +alpha*csi.*(3*ctilde.*eta./R.^5 -2*Z32 -Z0);                                % third row, first column
    jc2 =  (1-alpha)*2*Y0*sin(delta)                -dtilde./R.^3 +alpha*ctilde.*(1-3*q.^2./R.^2)./R.^3;                                % first row, first column
    jc3 = -(1-alpha)*(ytilde./R.^3 -Y0*cos(delta))                -alpha*(3*ctilde.*eta.*q./R.^5 -q.*Z0);                                  % second row, first column
% *************************************************************************
    % TENSILE Y-derivative: Okada (1992), Table 8, Type Tensile
    kc1 =  (1-alpha)*(q./R.^3 +Y0*sin(delta)*cos(delta))      +alpha*(z*cos(delta)./R.^3 +3*ctilde.*dtilde.*q./R.^5 -q.*Z0*sin(delta));                                       % first row, first column
    kc2 = -(1-alpha)*2*csi.*P*sin(delta) -ytilde.*dtilde.*X32 +alpha*ctilde.*((dtilde+2*q*sin(delta)).*X32 - ytilde.*q.^2.*X53);                                       % second row, first column
    kc3 = -(1-alpha)*(csi.*P*cos(delta) -X11 +ytilde.^2.*X32) +alpha*ctilde.*((dtilde+2*q*cos(delta)).*X32 - ytilde.*eta.*q.*X53) +alpha*csi.*Q;                                       % third row, first column
% *************************************************************************
end
