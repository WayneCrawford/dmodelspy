...
xci = M{1}; yci = M{2};
xcf = M{3}; ycf = M{4};
[xc yc zc uc vc wc] = textread('../coulomb/fault5.ds.z0.cou','%f %f %f %f %f %f','headerlines',3);
UCX = TriScatteredInterp(xc,yc,uc); UCY = TriScatteredInterp(xc,yc,vc); UCZ = TriScatteredInterp(xc,yc,wc); 

% displacement from okada92
mu=1; nu=0.25; 
x = (0:1:25); y = x+5;   % cross-section
[u9 v9 w9] = okada92fault('dip',xi,yi,xf,yf,zt,zb,Ud,delta,mu,nu,1000*xc,1000*yc,zc);
...
figure('Position',[100 scsz(4)/10 0.8*scsz(3) 0.5*scsz(4)],'Name','Dip slip fault (FREE SURFACE)');
subplot(1,4,[1 2]);
                plotpatches(xi,yi,xf,yf,zt,zb,delta,-7,22);
                xlabel('X (km)'); ylabel('Y (km)'); zlabel('Depth (km)');
                axis equal;
                title(sprintf('DIP SLIP deformation at %3.2f km',zc(1))); 
subplot(1,4,3);  
