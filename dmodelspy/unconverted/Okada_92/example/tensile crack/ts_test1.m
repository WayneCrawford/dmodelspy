...
% Tensile fault displacement from Coulomb 3.1
faultfid = fopen(faultinp,'r+');
M = textscan(faultfid,'%*f %f %f %f %f %*f %f %*f %f %f %f %*[^\n]','headerlines',2);
xci = M{1}; yci = M{2};
xcf = M{3}; ycf = M{4};
[xc yc zc uc vc wc] = textread('../coulomb/fault5.ts.z0.cou','%f %f %f %f %f %f','headerlines',3);
...
[u9 v9 w9] = okada92fault('tensile',xi,yi,xf,yf,zt,zb,Ud,delta,mu,nu,1000*xc,1000*yc,zc);
...
figure('Position',[100 scsz(4)/10 0.8*scsz(3) 0.5*scsz(4)],'Name','Tensile fault (FREE SURFACE)');
subplot(1,4,[1 2]);
                plotpatches(xi,yi,xf,yf,zt,zb,delta,-7,22);
                xlabel('X (km)'); ylabel('Y (km)'); zlabel('Depth (km)');
                axis equal;
                title(sprintf('TENSILE deformation at %3.2f km',zc(1))); 
subplot(1,4,3);  
